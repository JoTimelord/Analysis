...
namespace TWOFATJETSCUT{
    ...
    bool eq2ElectronsPtGt30(Nano& nt, Arbol& arbol, Cutflow& cutflow, bool sf, double pt);
    ...
    bool mjjGt500(Nano& nt, Arbol& arbol, Cutflow& cutflow, double mjj);
    bool deltaEtaGt3(Nano& nt, Arbol& arbol, Cutflow& cutflow, double mindeta);
...
void TWOFATJETSCUT::selectVBFs(int& i, int& j, std::vector<LorentzVector> jets) {
...
// Select Z->l+l-
bool TWOFATJETSCUT::eq2ElectronsPtGt30(Nano& nt, Arbol& arbol, Cutflow& cutflow, bool sf=true, double pt=30) {
    int n_leptons = 0;
    ...
    for (unsigned int elec_i = 0; elec_i < nt.Electron_pt().size(); ++elec_i)
    {
        ...
        if (ttH_UL::electronID(elec_i, ttH::IDtight, nt.year()) && this_pt > pt) { 
            n_leptons++; 
    ...
    for (unsigned int muon_i = 0; muon_i < nt.Muon_pt().size(); ++muon_i) {
        ...
        if (this_pt > pt && ttH_UL::muonID(muon_i, ttH::IDtight, nt.year())) { 
            n_leptons++; 
    ...
    if (os) {logic=(n_leptons==2 && (elec.size()==2 || muon.size()==2) && os<0);}
...
// Function to select VBF jet system mass larger than 500
bool TWOFATJETSCUT::mjjGt500(Nano& nt, Arbol& arbol, Cutflow& cutflow, double mjj=500) {
    bool logic = (cutflow.globals.getVal<double>("mjj")>=500);
    return logic;
}

// Function to select delta eta between VBF jets >= 3, then fill the selections
bool TWOFATJETSCUT::deltaEtaGt3(Nano& nt, Arbol& arbol, Cutflow& cutflow, double mindeta=3) {
