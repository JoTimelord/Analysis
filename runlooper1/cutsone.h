...
namespace ONEFATJETCUT{
    ...
    bool eq2ElectronsPtGt30(Nano& nt, Arbol& arbol, Cutflow& cutflow, double pt, bool sf);
...
// Select Z->l+l-
bool ONEFATJETCUT::eq2ElectronsPtGt30(Nano& nt, Arbol& arbol, Cutflow& cutflow, double pt=30, bool sf=true) {
    int n_leptons = 0;
    ...
    for (unsigned int elec_i = 0; elec_i < nt.Electron_pt().size(); ++elec_i)
    {
        ...
        if (ttH_UL::electronID(elec_i, ttH::IDtight, nt.year())) { 
        // if (pt > 30) {
            n_leptons++; 
    ...
    for (unsigned int muon_i = 0; muon_i < nt.Muon_pt().size(); ++muon_i) {
        ...
        if (ttH_UL::muonID(muon_i, ttH::IDtight, nt.year())) { 
            n_leptons++; 
...
bool ONEFATJETCUT::mjjGt500(Nano& nt, Arbol& arbol, Cutflow& cutflow, double mjj=500) {
    bool logic = (cutflow.globals.getVal<double>("mjj")>=mjj);
    return logic;
}

// Function to select delta eta between VBF jets >= 3, then fill the selections
bool ONEFATJETCUT::deltaEtaGt3(Nano& nt, Arbol& arbol, Cutflow& cutflow, double mindeta=3) {
